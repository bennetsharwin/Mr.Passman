...
# -> INSERT INTO data (username, password) VALUES ('main', 'password123')
# You will need to install this python library for mysql:
...
conn=mysql.connector.connect(user='passman', password='password123', host='127.0.0.1', database='program') #<- please enter your own username and password for mysql
...
def user_menu(user, passw):
	...
	if opt=='1':
		...
		T=input(bcolors.BLUE + 'type the site name of the credentials you want to remove: ' + bcolors.RESET)
		cr.execute("DELETE FROM data2 WHERE T=%s", (T,))
		print(bcolors.YELLOW + "Data Deleted." + bcolors.RESET)
		user_menu(user, passw)
	elif opt=='4':				#Something is wronmg with this part. must find it.
		current=getpass.getpass(bcolors.BLUE + "Currunt Password: " + bcolors.RESET)
		cr.execute('SELECT * FROM data;')
		for password in cr:		#Errors found
			x = encryptNow(current)		#Errors found
			if password==x:		#Errors found
				new_pass=getpass.getpass(bcolors.BLUE + "New Password: " + bcolors.RESET)#Errors found
				y = encryptNow(new_pass)#Errors found
				cr.execute("UPDATE data SET password=%s WHERE username=%s", (y,user))#Errors found
				print(bcolors.GREEN + "Password changed succesfully." + bcolors.RESET)#Errors found
				user_menu(user, y)#Errors found
			else:#Errors found
				print("Wrong Password!")#Errors found
	elif opt=='5':
...
def add_data(user,passw):
	T=input(bcolors.BLUE + "Enter the site name where the credentials belong to: " + bcolors.RESET)
...
def create_user(user, passw):
	x = encryptNow(passw)
	cr.execute("INSERT INTO data (username, password) VALUES (%s, %s)", (user, x))
	cr.execute("SELECT * FROM data WHERE username=%s AND password=%s", (user, x))
	for (username, password) in cr:
		if username == user and x==password:
...
def get_user(user, passw):
	...
	for (username, password) in cr:
		y = encryptNow(passw)
		if username==user and y==password:
			x=1
		elif username != user and y != password:	#Wrong Password and Username!
			x=2
		elif username == user and y != password:	#Wrong Password!
			x=3
		elif username != user and y == password:		#Wrong username!
			x=4
		else:
